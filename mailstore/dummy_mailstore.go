...
func (u *DummyUser) CreateMailboxByName(name string) (Mailbox, error) {
	if name == "invalid" {
		return nil, fmt.Errorf("name invalid")
	}
	mailbox := newDummyMailbox(name)
	u.mailboxes = append(u.mailboxes, mailbox)
	return mailbox, nil
}
...
func (u *DummyUser) DeleteMailboxByName(name string) (error) {
	result :=[]*DummyMailbox{}
	found := false
	for _,mailbox:= range u.mailboxes {
		if mailbox.name == name {
